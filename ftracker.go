...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Плавание":
		...
		calories := SwimmingSpentCalories(lengthPool, countPool, duration, weight, speed) // вызовите здесь необходимую функцию
		return fmt.Sprintf("Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n", trainingType, duration, distance, speed, calories)
	default:
...
func WalkingSpentCalories(action int, duration, weight, height, speed float64) float64 {
	// ваш код здесь
	speedMInS := speed * kmhInMsec // Преобразование скорости в м/с
	heightInM := height / cmInM    // Преобразование роста в метры
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMInS, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
